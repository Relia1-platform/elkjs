...
| `elk.geometric.tree.hanging` | `0` | A parent with more leaf children than this hangs them in columns beside vertical trunks, one leaf per side and row, connected through the bus, the trunk, and a stub that carries the label. A star with that many leaves becomes a TREE in AUTO mode. Compact and readable for hubs with many labeled devices. |
| `elk.geometric.packing` | `SHELF` | `COMPACT` arranges the connected components of a scope by decreasing height, first fit into the rows opened so far: aligned tops, full rows, a denser drawing, and an arrangement independent of the input order. |
| `elk.geometric.refine` | `false` | Polish the placed positions with small moves that keep spacing and order: straighten connectors whose ends almost share an axis, align nodes that almost share a row or column, equalize nearly equal gaps, and snap node borders to the grid. Works in every mode, including `FIXED` positions from `layered` or from the user. |
| `elk.geometric.refineGrid` | `1` | Grid in pixels that refined node borders snap to; `0` keeps positions off the grid. The final translation is rounded to the grid too, so returned coordinates stay on it. |
...
With `elk.geometric.tree.hanging: N` a parent with more than N leaf children gets an org-chart block: the leaves hang in columns beside vertical trunks, in rows of one leaf per side, as square a block as the counts allow. Each hanging edge goes from the parent through the shared bus, down its trunk, and across a stub into the leaf's side; the stub is long enough for the widest label in the block, so a hub with a hundred labeled devices measures about 900 by 1100 px instead of 5200 px square, and `junctionPoints` mark the trunk and stub branches. Other children of the parent keep their row beside the block, and in AUTO mode a star with more leaves than the threshold is drawn as such a tree.

With `elk.geometric.refine: 'true'` the placed positions are polished before routing, and `FIXED` positions from another algorithm or from the user can be polished too. Connectors whose ends are within a quarter of the node spacing (at least 4 px) of a shared axis are straightened, nodes that almost share a row or column are aligned, nearly equal gaps in a row or column become equal, and node borders snap to `elk.geometric.refineGrid`. Every move keeps the required spacing, never shrinks a gap that is already below it, keeps the relative order of nodes, and is taken only when straightness, alignment, equal gaps, or crispness improve by more than the distance moved costs; a node that moves onto an axis carries the neighbors whose connectors are straight or nearly straight on that axis, so staircases from layered placement are straightened level by level. The result is deterministic and, with `STABLE_ID`, independent of input order.
...
Shared fixtures are in `../elk/test/geometry`. JVM tests export results to `../elk/test/org.eclipse.elk.alg.geometric.test/target/geometric-results`; the parity script compares node, port, label, and edge coordinates with the freshly built JS bundle, using `1e-6 × max(1, diagram size)` tolerance. `node scripts/geometric-sweep.cjs [seed] [runs] [overlap]` lays out random trees, stars, rings, forests and meshes through ten feature combinations and checks containment, spacing, labels, finite orthogonal routes, and timing; with `overlap` the FIXED positions may overlap, which exercises the lenient fallbacks.
